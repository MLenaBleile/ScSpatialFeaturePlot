...
vignette: >
  %\VignetteIndexEntry{Plotting With ScSpatialFeaturePlot}
...
ScSpatialFeaturePlot(spat, features = c("MLANA", "MITF"),
                     pt.size.factor = 2) + ggplot2::ggtitle("ScSpatialFeaturePlot")

SpatialFeaturePlot(spat, features = c("MLANA", "SOX10"),
                   pt.size.factor = 2.8, slot = "counts") +
  ggplot2::ggtitle("SpatialFeaturePlot")
```


If you want to plot genes on different scales, just set combine = FALSE. This will return a list of ggplots which can be arranged with grid.arrange. I chose not to put the grid.arrange inside the function because it is sometimes useful to manipulate, replace, or add specific gg plots in the list before calling.

```{r multi}
myplots <- ScSpatialFeaturePlot(spat,
...
ScSpatialFeaturePlot(spat, features = c("VIM"),
                     pt.size.factor = 2,
                     cells = WhichCells(spat, expression = PTPRC == 0,
                                        slot = "counts"),
                     lq = 0.25, uq = 0.95)
```
